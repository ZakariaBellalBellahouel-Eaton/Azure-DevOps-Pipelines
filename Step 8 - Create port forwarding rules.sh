...
#Get the Ip Adresse
if [[ ! $(sudo iptables -C PREROUTING -t nat -i $HOSTNETWORKINTERFACENAME -p tcp --dport 13000 -j DNAT --to-destination $CONTAINERIPADDRESS:$CONTAINERPXMC3000APPLICATIONPORT  2>/dev/null) ]]; then
